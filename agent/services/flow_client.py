...
class FlowClient:
    ...
    async def _poll_batch_operation(self, operation_id: str) -> dict:
        ...
        try:
            ...
            # The listing is scanned with an 800-byte window, so a job whose
            # own media slot is still null can be handed a neighbour's id. An
            # id the media rpc denies is never going to resolve, and keeping it
            # cached poisons every later round into the same timeout.
            self._operation_media.pop(operation_id, None)
            return _as_pending_operation(operation_id, error=str(exc))
