...
# `generate` hands back a list of 1-D numpy waveforms; `torchaudio.save` wants a
# 2-D float tensor, so every call site goes through this.
_AS_WAV = """
def _as_wav(chunk):
    wav = chunk if torch.is_tensor(chunk) else torch.as_tensor(chunk)
...
torchaudio.save(args["output"], _as_wav(audio[0]), args["sample_rate"])
...
for item in args["items"]:
    try:
        ...
        torchaudio.save(item["output"], _as_wav(audio[0]), args["sample_rate"])
